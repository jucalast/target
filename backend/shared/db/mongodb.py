...
class MongoDBConnection:
    ...
    def _initialize_connection(self) -> None:
        ...
        try:
            ...
            # Configurações adicionais podem ser passadas via QUERY_STRING
            # Ex: mongodb://localhost:27017/?retryWrites=true&w=majority
            client_options = {
    ...
    def ensure_indexes(self) -> None:
        ...
        try:
            # Índices para a coleção de notícias
            news_collection = self.get_collection('news_articles')
            news_collection.create_index([('title', 'text'), ('content', 'text')])
            news_collection.create_index([('published_at', -1)])  # Ordenação por data
            news_collection.create_index([('source.domain', 1)])  # Filtro por fonte
            news_collection.create_index([('category', 1)])  # Filtro por categoria
            news_collection.create_index([('sentiment_score', 1)])  # Análise de sentimento
            
            # Índices para a coleção de métricas
