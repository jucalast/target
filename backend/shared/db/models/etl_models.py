...
etl_result_news = Table(
    'etl_result_news',
    Base.metadata,
    Column('etl_result_id', PG_UUID(as_uuid=True), ForeignKey('etl_results.id'),\
        primary_key=True),
...
class DataPointModel(Base):
    ...
    meta_info = Column('metadata', JSONB,\
        default={})  # Renomeado para evitar conflito com SQLAlchemy
...
class MarketMetricModel(Base):
    ...
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow,\
        onupdate=datetime.utcnow)
    # Relacionamentos
    segment_id = Column(PG_UUID(as_uuid=True), ForeignKey('market_segments.id'),\
        nullable=False)
...
class MarketSegmentModel(Base):
    ...
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow,\
        onupdate=datetime.utcnow)
    # Relacionamentos
    etl_result_id = Column(PG_UUID(as_uuid=True), ForeignKey('etl_results.id'),\
        nullable=False)
    metrics = relationship("MarketMetricModel", backref="segment", cascade="all,\
        delete-orphan")
...
class SearchTrendModel(Base):
    ...
    values = Column(JSONB,\
        nullable=False)  # Lista de dicionários {date: datetime, value: float}
...
class ETLResultModel(Base):
    ...
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow,\
        onupdate=datetime.utcnow)
    # Relacionamentos
    market_segments = relationship("MarketSegmentModel", backref="etl_result",\
        cascade="all, delete-orphan")
    search_trends = relationship("SearchTrendModel", backref="etl_result",\
        cascade="all, delete-orphan")
    news_articles = relationship("NewsArticleModel", secondary=etl_result_news,\
...
class ETLRunLogModel(Base):
    ...
    status = Column(String(50), nullable=False)  # 'started', 'completed',\
        'failed'
