...
# entre ETLResult e NewsArticle
etl_result_news = Table(
    'etl_result_news',
    Base.metadata,
    Column('etl_result_id', PG_UUID(as_uuid=True), ForeignKey('etl_results.id'), primary_key=True),
    Column('news_article_id', PG_UUID(as_uuid=True), ForeignKey('news_articles.id'), primary_key=True)
)
...
class DataPointModel(Base):
    ...
    meta_info = Column('metadata', JSONB, default={})  # Renomeado para evitar conflito com SQLAlchemy
    ...
    def __repr__(self):
        return f"<DataPoint(id={self.id}, value={self.value}, source={self.source})>"
...
class MarketSegmentModel(Base):
    ...
    # Relacionamentos
    etl_result_id = Column(PG_UUID(as_uuid=True), ForeignKey('etl_results.id'), nullable=False)
    metrics = relationship("MarketMetricModel", backref="segment", cascade="all, delete-orphan")
    
    def __repr__(self):
        return f"<MarketSegment(id={self.id}, name={self.name}, etl_result_id={self.etl_result_id})>"
...
class NewsArticleModel(Base):
    ...
    # Relacionamento muitos-para-muitos com ETLResult
    etl_results = relationship("ETLResultModel", secondary=etl_result_news, back_populates="news_articles")
...
class SearchTrendModel(Base):
    ...
    values = Column(JSONB, nullable=False)  # Lista de dicionários {date: datetime, value: float}
    related_queries = Column(JSONB, default=[])  # Lista de dicionários
    interest_by_region = Column(JSONB, default=[])  # Lista de dicionários
    created_at = Column(DateTime(timezone=True), default=datetime.utcnow)
    updated_at = Column(DateTime(timezone=True), default=datetime.utcnow, onupdate=datetime.utcnow)
    ...
    def __repr__(self):
        return f"<SearchTrend(id={self.id}, keyword={self.keyword}, etl_result_id={self.etl_result_id})>"
...
class ETLResultModel(Base):
    ...
    # Relacionamentos
    market_segments = relationship("MarketSegmentModel", backref="etl_result", cascade="all, delete-orphan")
    search_trends = relationship("SearchTrendModel", backref="etl_result", cascade="all, delete-orphan")
    news_articles = relationship("NewsArticleModel", secondary=etl_result_news, back_populates="etl_results")
    
    def __repr__(self):
        return f"<ETLResult(id={self.id}, request_id={self.request_id}, timestamp={self.timestamp})>"
...
class ETLRunLogModel(Base):
    ...
    def __repr__(self):
        return f"<ETLRunLog(id={self.id}, request_id={self.request_id}, status={self.status})>"
