...
class BasePostgresRepository(Generic[ModelType, CreateSchemaType, \
    UpdateSchemaType]):
    """
    ...
    def __init__(self, model: Type[ModelType], \
        ...
        model: Classe do modelo SQLAlchemy
            db_session: Sessão do SQLAlchemy (opcional, \
                usa a conexão padrão se não fornecido)
        """
        self.model = model
    ...
    """
        ...
        try:
            ...
            obj_data = obj_in.dict() if hasattr(obj_in, \
                'dict') else dict(obj_in)
        ...
        self.db.rollback()
            ...
            raise ValidationError(\
                "Erro de integridade nos dados fornecidos") from e
    ...
    def list(self, skip: int = 0, limit: int = 100, \
        **filters) -> List[ModelType]:
        ...
        query = self.db.query(self.model)
            ...
            query = query.filter(getattr(self.model, \
                            field).in_(value))
                    else:
                    query = query.filter(getattr(self.model, \
                            field) == value)
    ...
    """
        ...
        id: ID do registro a ser atualizado
            obj_in: Dados para atualização (\
                pode ser um dicionário ou um schema Pydantic)
            exclude_unset: Se True,
                ignora campos não fornecidos (padrão: True)
        Returns:
        ...
        self.db.rollback()
            ...
            raise DatabaseError(\
                f"Erro ao atualizar registro com ID {id}") from e
    ...
    """
        ...
        self.db.rollback()
            logger.error(f"Erro ao executar consulta SQL: {str(\
                e)}\nSQL: {sql}")
