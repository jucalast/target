...
class HealthCheckResult(BaseModel):
    ...
    status: str = Field(..., description="Status da verificação (healthy, degraded, unhealthy)")
    ...
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Data e hora da verificação")

class HealthStatus(BaseModel):
    """Modelo para o status geral de saúde da aplicação."""
    status: str = Field(..., description="Status geral da aplicação (healthy, degraded, unhealthy)")
    version: str = Field(..., description="Versão da aplicação")
    timestamp: datetime = Field(default_factory=datetime.utcnow, description="Data e hora da verificação")
    checks: Dict[str, HealthCheckResult] = Field(default_factory=dict, description="Resultados das verificações individuais")
...
def check_postgres_health() -> HealthCheckResult:
    ...
    try:
        ...
        logger.error(f"Erro ao verificar saúde do PostgreSQL: {str(e)}", exc_info=True)
        return HealthCheckResult(
            status="unhealthy",
            component="postgresql",
            details={"error": str(e)}
...
def check_mongodb_health() -> HealthCheckResult:
    ...
    try:
        ...
        logger.error(f"Erro ao verificar saúde do MongoDB: {str(e)}", exc_info=True)
        return HealthCheckResult(
            status="unhealthy",
            component="mongodb",
            details={"error": str(e)}
...
def check_disk_usage() -> HealthCheckResult:
    ...
    try:
        ...
        logger.error(f"Erro ao verificar uso do disco: {str(e)}", exc_info=True)
        return HealthCheckResult(
            status="unhealthy",
            component="disk",
            details={"error": str(e)}
...
def check_memory_usage() -> HealthCheckResult:
    ...
    try:
        ...
        logger.error(f"Erro ao verificar uso de memória: {str(e)}", exc_info=True)
        return HealthCheckResult(
            status="unhealthy",
            component="memory",
            details={"error": str(e), "message": "psutil não instalado ou erro na leitura"}
        )
...
def get_health_status(version: str = "1.0.0") -> HealthStatus:
    ...
    # Determina o status geral com base nas verificações
...
def health_check_dependency(version: str = "1.0.0"):
    ...
    # Define quais componentes são críticos
    critical_components = ["postgresql"]
    
    # Verifica se algum componente crítico está com problemas
