...
class KeywordFeature(BaseModel):
    ...
    method: str = Field(..., description="Método de extração (ex: 'tfidf', 'lda')")
    
class EntityFeature(BaseModel):
    """Entidade nomeada extraída do texto."""
    text: str = Field(..., description="Texto da entidade")
    label: str = Field(..., description="Tipo da entidade (ex: 'PERSON', 'ORG')")
...
class TopicFeature(BaseModel):
    ...
    keywords: List[Dict[str, Any]] = Field(..., description="Palavras-chave do tópico com escores")
...
class NLPFeatures(BaseModel):
    ...
    entities: List[EntityFeature] = Field(default_factory=list, description="Entidades nomeadas identificadas")
    topics: List[TopicFeature] = Field(default_factory=list, description="Tópicos identificados")
        ...
        schema_extra = {
            "example": {
                ...
                "keywords": [
                    ...
                    {"keyword": "pequenas empresas", "score": 0.87, "method": "tfidf"}
                ],
                "entities": [
                    {"text": "tecnologia", "label": "TECHNOLOGY", "start_char": 20, "end_char": 30}
                ],
                "topics": [
                    {
                        "topic_id": 0,
                        "keywords": [{"word": "tecnologia", "score": 0.8}, {"word": "inovação", "score": 0.7}],
                        "score": 0.92
                    }
                ],
                "embeddings": {
