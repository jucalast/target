...
class IBGETableQuery(BaseModel):
    ...
    variables: List[str] = Field(...,\
        description="Lista de códigos de variáveis")
    ...
    period: str = Field("last", description="Período dos dados (ex: 'last',\
        '2022')")
...
class GoogleTrendsQuery(BaseModel):
    ...
    timeframe: str = Field("today 12-m",\
        description="Período de tempo para a análise")
    geo: str = Field("BR", description="Localização geográfica")
    gprop: str = Field("web", description="Fonte dos dados (web, news, images,\
        youtube)")

class NewsScrapingQuery(BaseModel):
    """Parâmetros para coleta de notícias."""
    keywords: List[str] = Field(...,\
        description="Termos de busca para as notícias")
    sources: Optional[List[str]] = Field(
        None,
        description="Fontes de notícias específicas (opcional)"
    )
    max_articles: int = Field(20,\
        description="Número máximo de artigos para coletar")
    timeframe_days: int = Field(30,\
        description="Número máximo de dias para trás")
...
class ETLParameters(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.utcnow,\
        description="Data/hora da requisição")
    ...
    nlp_features: NLPFeatures = Field(...,\
        description="Recursos de NLP extraídos do texto")
    ...
    max_retries: int = Field(3,\
        description="Número máximo de tentativas para cada requisição")
