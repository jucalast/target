...
def fetch_common_queries(service: SIDRAService, cache_dir: Path):
    ...
    for location in locations:
        for age_range in age_ranges:
            cache_file = cache_dir / "pof" / f"pop_{location}_{age_range[0]}_{age_range[1]}.json"
            if not cache_file.exists():
                try:
                    logger.info(f"Buscando dados populacionais para {location}, faixa {age_range[0]}-{age_range[1]} anos")
                    data = service.get_population_by_age_range(location, age_range)
                    with open(cache_file, 'w', encoding='utf-8') as f:
                        json.dump(data, f, ensure_ascii=False, indent=2)
                except Exception as e:
                    logger.error(f"Erro ao buscar dados para {location}, {age_range}: {str(e)}")
    ...
    for location in locations:
        ...
        if not cache_file.exists():
            try:
                logger.info(f"Buscando dados de consumo cultural para {location}")
                data = service.get_consumer_behavior(location)
                with open(cache_file, 'w', encoding='utf-8') as f:
                    json.dump(data, f, ensure_ascii=False, indent=2)
            except Exception as e:
                logger.error(f"Erro ao buscar dados de cultura para {location}: {str(e)}")
    ...
    for location in locations:
        ...
        if not cache_file.exists():
            try:
                logger.info(f"Buscando dados de distribuição de renda para {location}")
                data = service.get_income_distribution(location)
                with open(cache_file, 'w', encoding='utf-8') as f:
                    json.dump(data, f, ensure_ascii=False, indent=2)
            except Exception as e:
                logger.error(f"Erro ao buscar dados de renda para {location}: {str(e)}")
